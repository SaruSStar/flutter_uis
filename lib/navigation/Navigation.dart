...
class _HiddenMenuPageState extends State<HiddenMenuPage> with
    TickerProviderStateMixin {

  TextStyle titile =TextStyle(
  ...
  void initState() {
    super.initState();
    openController = AnimationController(duration: const Duration(
        milliseconds: 200), vsync: this);
    closeController = AnimationController(duration: const Duration(
        milliseconds: 200), vsync: this);
    ...
    return Scaffold(
      ...
      body: SafeArea( child: Stack(
          ...
          children: <Widget>[ Container(
              color: Colors.pink, height: menuHeight,
              child: Column( children: <Widget>[
                  SizedBox(height: 10.0,), Row(
                    children: <Widget>[
                      IconButton(icon: Icon(menuShown ? Icons.cancel
                          : Icons.menu), color: Colors.white,
                        onPressed: _handleMenuPress,),
                      Text(menuShown ? "Menu" :"Home", style: titile),
                      Padding(child:
                      FlatButton(child: Text("Code  ",style: titile,),
                      onPressed: (){Navigator.push(context, MaterialPageRoute
                        (builder: (context){
                          return CodeView("code/hide.jpg");
                        }));
                      },),
                  ...
                  Expanded( child: ListView(
                      children: <Widget>[ Container(
                          padding: EdgeInsets.only(bottom: 10.0),
                            ...
                            child: Text("Home", style: titile
                            )
                        ),
                        Container( padding: EdgeInsets.only(bottom: 10.0),
                            alignment: Alignment.center,
                            child: Text("Cart", style: titile)
                        ),
                        Container( padding: EdgeInsets.only(bottom: 10.0),
                            alignment: Alignment.center,
                            child: Text("Wishlist", style: titile)
                        ),
                        Container( padding: EdgeInsets.only(bottom: 10.0),
                            alignment: Alignment.center,
                            child: Text("Profile", style: titile)
                        ),],),
                  )],),
            ),
            LayoutBuilder(
              builder: (context, constraints) {
                return Container(
                  margin: EdgeInsets.only(top: menuHeight * 
                      (constraints.maxHeight - 60) + 60),
                  color: Colors.transparent, child: Material(
                    elevation: 16.0,  shape: BeveledRectangleBorder(
